...
namespace CMS.Application.Auth.Services
{
    public class AuthService : IAuthService
    {
        ...
        public async Task SendForgotPasswordOtpAsync(string email)
        {
            ...
            await _verificationCodeRepository.CreateAsync(verification);
            var resetLink = $"http://localhost:4200/auth/reset-password?email={email}&code={code}";
            var message = $"Your password reset code is: {code}. You can also reset your password by clicking here: {resetLink}";
            var htmlMessage = $"<p>Your password reset code is: <strong>{code}</strong></p><p>Alternatively, you can click the link below to reset your password:</p><p><a href='{resetLink}'>Reset Password</a></p>";
            await _emailService.SendEmailAsync(email, "Password reset code", message, htmlMessage);
        }
        ...
        public async Task AcceptInvitationAsync(string token, string newPassword)
        {
            ...
            {
                var existingDoctor = await _doctorRepository.GetDoctorByIdAsync(user.UserID);
                if (existingDoctor == null)
                {
                    await _doctorRepository.CreateAsync(new Doctor
                    {
                        DoctorID = user.UserID,
                        Specialization = "To be filled",
                        Qualification = "To be filled",
                        YearOfExperience = 0,
                        WorkingDays = Array.Empty<WorkingDays>(),
